...
console.log("Task 1: accessing arrays\n")
...
console.log("Task 2: Fixing a typo\n")
...
console.log("Task 3: Get artist by Index function\n")

function getArtistByIndex(targetArray,targetIndex) {
  const result = `the artist at index ${targetArray[targetIndex]["id"]} is ${targetArray[targetIndex]["name"]}`
  console.log(result,"\n")
  return result
}
...
function listOfNames(targetArray) {
  const listOfNames = []
  for (let i = 0; i < (targetArray.length); i++){
    listOfNames.push(targetArray[i]["name"])
  }
  console.log(`Generating a list of names for the requested array...\n`)
  return listOfNames
}
...
function removeArtist(targetArray,targetIndex) {
  const removeArtist = []
  for (let i = 0; i < targetArray.length; i++) {
    removeArtist.push(targetArray[i])
  }
  removeArtist.splice(targetIndex,1)
  console.log("Removing element in requested array @ requested index...\n")
  return removeArtist
}
...
function addArtist(targetArray, name, years, genre, nationality, bio) {
  const newArtist = {
    "name": name,
    "years": years,
    "genre": genre,
...
function lotsOfArt(targetArray) {
  const prolific = []
  for (let i = 0; i < targetArray.length; i++) {
    if (targetArray[i]["paintings"] > 100) {
      prolific.push(targetArray[i]["name"])
    }
  }
  return prolific
}
...
function artistInfo(targetArray,targetArtist){
    for (let i = 0; i < targetArray.length; i++) {
      if (targetArray[i]["name"] === targetArtist) {
        return targetArray[i]["bio"]
      }
    }
}
...
function artistByCountry(targetArray,targetNationality) {
  const artistsOfNationality = []
  for (let i = 0; i < targetArray.length; i++) {
    if (targetArray[i]["nationality"] === targetNationality) {
      artistsOfNationality.push(targetArray[i]["name"])
    }
  }
  return artistsOfNationality
}
